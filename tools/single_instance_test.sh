...
# === 1. 灌数据（直连模式，80 workers × 4600 × 4MB = 1.41TB） ===
PREFIX="single"
VALUE_SIZE=4194304
WORKERS=80
COUNT=4600
...
./bench --mode write --raw --direct-addr 127.0.0.1:33000 \
  ...
  --count-per-worker $COUNT --prefix $PREFIX > /root/write_single.log 2>&1
RC=$?
echo "write exit=$RC elapsed=$(( $(date +%s) - START ))s"
grep -E 'Result|elapsed|errors|throughput' /root/write_single.log
...
# 读测试
START=$(date +%s)
timeout 70 ./bench --mode read --into --direct-addr 127.0.0.1:33000 \
  --workers 80 --duration 60s --value-size $VALUE_SIZE \
...
SB=$(awk '{print $2}' $OUT/stat_before_nvme1n1)
...
echo ""
echo "Output: $OUT"
echo "===== SINGLE INSTANCE TEST DONE ====="